...
export const uploadFile = async (req, res, next) => {
  try {
    ...
    console.log('finalFolderId after parsing:', finalFolderId);
...
// ============================================
// LIST FILES
// Mode MOBILE (défaut)  : data = [ {...file, isFavorite} ]  (flat, count)
// Mode WEB (?paginated=true ou ?page=)  : data = [...], pagination
// Inclut TOUJOURS le flag isFavorite (apport WEB) sans casser le mobile.
// ============================================
...
export const restoreFile = async (req, res, next) => {
  try {
    ...
    const fileToRestore = await prisma.file.findFirst({
...
// ============================================
// GET TRASH (Corbeille)
// Mode MOBILE (défaut)  : data = { folders, files } (tout, non paginé)
// Mode WEB (?paginated=true ou ?page=)  : pagination combinée folders+files triés par updatedAt
// ============================================
export const getTrash = async (req, res, next) => {
  try {
    ...
    // Mode paginé combiné (apport WEB)
    const page = parseInt(req.query.page) || 1;
...
export const emptyTrash = async (req, res, next) => {
  try {
    ...
    // Récupérer tous les fichiers supprimés (y compris ceux dans des dossiers supprimés)
    ...
    // Transaction pour supprimer dans le bon ordre (fichiers d'abord, puis dossiers)
    await prisma.$transaction(async (tx) => {
      // 1. Supprimer tous les fichiers supprimés
      await tx.file.deleteMany({ where: { ownerId: userId, isDeleted: true } });
      
      // 2. Supprimer tous les dossiers supprimés (les sous-dossiers seront supprimés en cascade)
      await tx.folder.deleteMany({ where: { ownerId: userId, isDeleted: true } });
      
      // 3. Mettre à jour le quota de l'utilisateur
    ...
    // Nettoyage physique des fichiers
    ...
    console.log(`Trash emptied: ${filesToDelete.length} files, ${foldersToDelete.length} folders deleted`);
